 ...
 * pour valider les fonctionnalités de la classe {@link KdTree}. Ces tests portent
 * notamment sur :
 * </p>
 * <ul>
 *   <li>Le chargement des données depuis des fichiers</li>
 *   <li>L'ajout (insertion) de points et la reconstruction de l'arbre</li>
 *   <li>Les recherches de min et max sur les deux premières dimensions</li>
 *   <li>La recherche par intervalle (range search)</li>
 *   <li>L'exécution de requêtes SQL simples</li>
...
public class KdTreeTest {
    ...
    /**
     * L'instance de {@link KdTree} testée dans chaque méthode.
     */
    ...
    /**
     * Renvoie le chemin absolu d'un fichier de test localisé dans le dossier des ressources de test.
     *
     * @param filename
     *         Le nom du fichier (ex. "1.txt").
     * @return Le chemin absolu du fichier, si trouvé dans les ressources (classpath).
     */
    ...
    /**
     * Teste le chargement d'un échantillon depuis un fichier texte.
     * <p>
     * Le fichier contient un nombre de points connu ; on vérifie que le {@link KdTree#nodeList}
     * correspond à la taille attendue.
     * </p>
     *
     * @throws IOException
     *         si le fichier est introuvable ou si une erreur de lecture survient.
     * @throws InvalidCriteriaException
     *         si le nombre de critères dans le fichier est invalide.
     */
    @Test
     ...
     * <p>
     * On définit d'abord {@link KdTree#criteriaNames}, puis on ajoute un point avec
     * {@code addPointAndRebuild}. On vérifie ensuite que la liste des nœuds contient
     * le point ajouté.
     * </p>
     *
     * @throws InvalidCriteriaException
     *         si le nombre de critères fournis à l'insertion est incorrect.
     */
    @Test
    ...
    /**
     * Teste la recherche de la valeur minimale pour chaque dimension dans un arbre préalablement chargé.
     *
     * @throws IOException
    ...
    /**
     * Teste la recherche de la valeur maximale pour chaque dimension dans un arbre préalablement chargé.
     *
     * @throws IOException
     *         si le fichier est introuvable.
     * @throws InvalidCriteriaException
     *         si le fichier contient des critères invalides.
     */
    @Test
    ...
    /**
     * Teste la recherche par intervalle (range search) avec des bornes spécifiques.
     *
     * @throws IOException
    ...
    public void testRangeSearch() throws IOException, InvalidCriteriaException {
        ...
        // Vérifie que tous les points de results respectent la plage
    ...
    /**
     * Teste l'exécution d'une requête SQL simple sur l'arbre chargé.
     *
     * @throws IOException
     *         si problème de fichier.
     * @throws InvalidCriteriaException
    ...
    /**
     * Teste l'exécution d'une requête SQL avec plusieurs conditions (AND).
     *
     * @throws IOException
     *         si problème de lecture de fichier.
     * @throws InvalidCriteriaException
     *         si les critères sont invalides.
     */
    @Test
    public void testExecuteQueryWithMultipleConditions() throws IOException, InvalidCriteriaException {
        ...
        expected1.put("department", "mathématique");
    ...
    /**
     * Teste la recherche par intervalle qui ne retourne aucun résultat.
     */
    @Test
    ...
    /**
     * Teste la recherche par intervalle quand plusieurs points partagent la même valeur
    ...
    /**
     * Teste l'exécution de requêtes SQL sur un fichier avec 3 critères (fichier 2.txt).
     */
    @Test
    ...
    /**
     * Teste la recherche par intervalle (range search) avec des données du fichier 2.txt.
     */
    @Test
    ...
    /**
     * Teste la recherche par intervalle avec des valeurs hors bornes (aucun résultat attendu).
     */
    @Test
    ...
    /**
     * Teste l'exécution de requêtes SQL avec les opérateurs >= et <=.
     */
    @Test
    ...
    /**
     * Teste qu'une insertion de points en double (mêmes coordonnées) lève une exception,
     * car les doublons sont interdits.
     *
     * @throws InvalidCriteriaException
     *         si les critères supplémentaires sont invalides.
     */
    @Test
    public void testInsertDuplicateValues() throws InvalidCriteriaException {
        ...
        // Tentative d'insertion d'un point aux mêmes coordonnées
        assertThrows(IllegalArgumentException.class, () -> {
            kdTree.addPointAndRebuild(new double[]{2.0, 170.0}, new String[]{"mathématique"});
        });

        // Vérification qu'il n'y a qu'un seul point dans l'arbre
        List<KdTreeNode> results =
    ...
    /**
     * Teste l'exécution de requêtes SQL avec des critères non numériques (opérateur "=").
     *
     * @throws IOException
    ...
    /**
     * Teste la levée d'exception lorsque le nombre de critères supplémentaires
     * ne correspond pas à {@link KdTree#criteriaNames}.
     */
    @Test
    public void testAddPointWithInvalidCriteria() {
        kdTree.criteriaNames = new String[]{"year", "height", "department"};
        assertThrows(InvalidCriteriaException.class, () -> {
            // Pas assez de critères texte
     ...
     * Vérifie que l'arbre contient bien tous les points et que min < max sur la dimension 0.
     *
     * @throws InvalidCriteriaException
     *         si un problème de critères survient lors des insertions.
     */
    @Test
    ...
    /**
     * Teste la méthode {@link KdTree#printTree()} pour vérifier qu'elle ne lance pas d'exception
    ...
    /**
     * Teste la recherche par intervalle avec des bornes égales (low == high) sur x et y,
     * pour repérer un point exact.
     *
     * @throws IOException
     *         si fichier introuvable.
     * @throws InvalidCriteriaException
    ...
    /**
     * Teste l'exécution d'une requête SQL avec un opérateur non supporté ("<>").
     *
     * @throws IOException
     *         si problème de fichier.
     * @throws InvalidCriteriaException
     *         si fichier mal formé.
     */
    @Test
     ...
     * On s'attend à une {@link NullPointerException}.
     */
    @Test
    ...
    /**
     * Teste le chargement d'un fichier au format invalide (ex. critère manquant).
     */
    @Test
    ...
    /**
     * Teste la sauvegarde dans un fichier sans permission d'écriture (ex. répertoire protégé).
     */
    @Test
    public void testSaveToFileWithoutWritePermission() throws IOException, InvalidCriteriaException {
        ...
        // Tentative de sauvegarde dans "/root/outputData.txt" qui, sur certains systèmes,
        // devrait lever une IOException (pas de droit d'écriture).
    ...
    /**
     * Teste l'appel à {@link KdTree#printTree()} lorsque l'arbre n'est pas construit,
    ...
    public void testPrintTreeWhenTreeNotBuilt() {
        ...
        assertEquals("Impossible d'afficher l'arbre : l'arbre n'a pas été construit car l'échantillon est vide.",
    ...
    public void testSaveSampleWithEmptyTree() {
        ...
        assertEquals("Impossible de sauvegarder : l'échantillon est vide.", exception.getMessage());
    }
}
